...
export const LoadingDots = ({ color }) => {
  const dotColor = color || COLORS.brightRed;
  ...
  useEffect(() => {
    const dotShowAnimations = dotOpacities.map((opacity) =>
      createTimingAnim(opacity, 1, 300)
    );

    const dotSHideAnimations = dotOpacities.map((opacity) =>
      createTimingAnim(opacity, 0, 300)
    );

    const sequence = Animated.sequence([
      Animated.stagger(200, dotShowAnimations),
      Animated.delay(200),
      // Animated.parallel(dotSHideAnimations),
      Animated.stagger(200, dotSHideAnimations),
      Animated.delay(200),
    ]);
